...
class Gui:
    def __init__(self):
        ...
        self.BACK = pygame.image.load("C:/Users/reignO7/Desktop/reign/CSC 171/171 assignments/CSC171-Activities-and-Assignments/Assignment 4/image/othello.jpg")
        self.BLUE = (0, 0, 255)
        self.YELLOW = (128, 128, 0)

        # display
        # self.SCREEN_SIZE = (641, 481)
        self.SCREEN_SIZE = (641, 481)
        ...
        self.board_img = pygame.image.load(os.path.join(
            "image", "board.bmp")).convert()
        self.black_img = pygame.image.load(os.path.join(
            "image", "preta.bmp")).convert()
        self.white_img = pygame.image.load(os.path.join(
            "image", "branca.bmp")).convert()
        self.tip_img = pygame.image.load(os.path.join("image",
                                                      "tip.bmp")).convert()
        self.clear_img = pygame.image.load(os.path.join("image",
                                                        "nada.bmp")).convert()
    ...
    def show_options(self):
        ...
        while True:
            ...
            title_fnt = pygame.font.SysFont("Boogaloo", 34)

            title = title_fnt.render("", True, self.WHITE)
            ...
            player1_txt = self.font.render("First Player", True, self.BLACK)
            player1_pos = player1_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=260)
            player2_txt = self.font.render("Second Player", True, self.BLACK)
            player2_pos = player2_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=300)
            level_txt = self.font.render("Computer Level", True, self.BLACK)
    ...
    def get_chosen_player(self):
        ...
        while True:
            self.screen.blit(self.BACK, [0, 0])
            # self.screen.fill(self.BACKGROUND)
            title_fnt = pygame.font.SysFont("comicsansms", 32)
            title = title_fnt.render("", True, Color(0, 0, 255))
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)
            human_txt = self.font.render("-> Human", True, self.BLACK)
            human_pos = human_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=250)
            comp_txt = self.font.render("-> Computer", True, self.BLACK)
    ...
    def get_chosen_level(self):
        ...
        while True:
            ...
            # self.screen.fill(self.BACKGROUND)
            # title_fnt = pygame.font.SysFont("Times New Roman", 34)
            title_fnt = pygame.font.SysFont("comicsansms", 72)
            title = title_fnt.render("", True, self.BLUE)
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=60)
            one_txt = self.font.render("-> Level 1", True, self.BLACK)
            one_pos = one_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=220)
            two_txt = self.font.render("-> Level 2", True, self.BLACK)
            two_pos = two_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=260)

            three_txt = self.font.render("-> Level 3", True, self.BLACK)
